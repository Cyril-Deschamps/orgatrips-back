...
import { getCityAverageRangeAccomodationsPrice } from "../app/services/accomodationPriceService";
...
program.command("insert").action(async () => {
  ...
  if (command.confirm) {
    ...
    cities = cities.filter((city) => !dbCities.find((dbCity) => dbCity.name === city.name));
    ...
    const promisesList = cities.map((city) => async () => {
      const [soloPricePerPerson, multiplePricePerPerson] = await Promise.all([getCityAverageRangeAccomodationsPrice(
        {
          adultsNumber: 1,
          city: city.name,
      ...
      ).then((ranges) => ranges.soloPricePerPersonRange), getCityAverageRangeAccomodationsPrice(
        {
          adultsNumber: 2,
          city: city.name,
          startDate: dateInThreeMonths,
